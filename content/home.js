...
},
{
   ...
   <p>Each time it's the computer's turn, the computer runs a number of simulated games from every possible position.  Specifically, for each open position, the computer makes a new board in memory and then moves to that position.  From there, it simulates finishing the game multiple times after making that move on this new board in memory by randomly placing Xs and Os.  It keeps track of how many games it has won and lost after trying each position, and then chooses its next spot based on the position that resulted in the highest frequency of won games.</p>
   <p>The hint feature for the human player uses the same algorithm: hover over 'Hint?' and you'll see the board highlight what's its determined to be the best and worst possible moves for you to make based on a number of simulations.  The hint feature is a little buggy at the moment.</p>
